...
int runepkg_config_load() {
    ...
    if (!config_file_path) {
        ...
        if (!g_runepkg_lists_dir) {
            fprintf(stderr, "Error: Failed to create lists directory path.\n");
            runepkg_config_cleanup();
            return -1;
        }
        g_download_dir = runepkg_util_concat_path(g_runepkg_base_dir, "download_dir");
        if (!g_download_dir) {
            fprintf(stderr, "Error: Failed to create default download_dir path.\n");
            runepkg_config_cleanup();
            return -1;
        }
        ...
        if (!g_download_dir) {
            fprintf(stderr, "Error: Failed to determine download_dir.\n");
            runepkg_util_free_and_null(&config_file_path);
            runepkg_config_cleanup();
            return -1;
        }
    ...
    if (g_sources) {
        for (int i = 0; i < g_sources_count; i++) {
            if (g_sources[i]) {
                ...
                free(g_sources[i]->suite);
                free(g_sources[i]->components);
                free(g_sources[i]);
            }
        }
        free(g_sources);
    ...
    while (fgets(line, sizeof(line), file)) {
        ...
        if (strncmp(trimmed, "deb ", 4) == 0 || strncmp(trimmed, "deb-src ", 8) == 0) {
            ...
            if (type && url && suite) {
                ...
                g_sources[g_sources_count] = malloc(sizeof(RuneSource));
...
void runepkg_init_paths() {
    ...
    if (!runepkg_util_file_exists(g_runepkg_lists_dir)) {
        if (runepkg_util_create_dir_recursive(g_runepkg_lists_dir, 0755) != 0) {
            fprintf(stderr, "Error: Failed to create lists directory %s\n", g_runepkg_lists_dir);
            runepkg_config_cleanup();
            exit(EXIT_FAILURE);
        }
    }

    if (!runepkg_util_file_exists(g_download_dir)) {
        if (runepkg_util_create_dir_recursive(g_download_dir, 0755) != 0) {
