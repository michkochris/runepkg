...
/**
 * @brief Expands ${VAR} and ${pwd} in a string.
 * @param input The string to expand.
 * @return A newly allocated expanded string, or NULL on error. Caller must free.
 */
...
/**
 * @brief Resolves a build target (dir or pkg name) to an absolute path.
 * Checks for direct path first, then looks in build_dir.
 */
const char *runepkg_util_resolve_build_target(const char *arg, const char *build_dir);

/**
 * @brief Calculates the system load factor (load average / processor count).
 * @return Load factor as a float, or -1.0f on error.
 */
float runepkg_util_get_load_factor(void);

/**
 * @brief Extracts ELF shared library dependencies (DT_NEEDED) using readelf.
 * @param elf_path Path to the ELF binary.
 * @param readelf_bin Path to the cross-readelf binary (optional).
 * @param out_deps Pointer to an array of strings that will be allocated.
 * @param out_count Pointer to an int to store the count.
 * @return 0 on success, -1 on failure.
 */
