...
extern bool g_verify_signatures; // Defined in runepkg_config.c or similar
...
int runepkg_crypto_verify_file(const char *file_path, const char *signature_path) {
    ...
    // We use --status-fd to get machine-readable output if needed,
    ...
    int result = runepkg_util_execute_command_silent(gpg_path, argv);

    char* pkg_name = get_pkg_name_from_path(file_path);
    if (result == 0) {
