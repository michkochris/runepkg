...
int runepkg_storage_list_packages(const char *pattern) {
    ...
    for (d = 0; d < 2; d++) {
        ...
        while ((entry = readdir(dir)) != NULL) {
            ...
            if (entry->d_type == DT_UNKNOWN) {
                ...
                struct stat st;
                snprintf(full, sizeof(full), "%.*s/%s", (int)(sizeof(full)-258), cdir, entry->d_name);
                if (stat(full, &st) == 0) {
            ...
            if (ver_dash && ver_dash != entry->d_name) {
                size_t nlen = (size_t)(ver_dash - entry->d_name);
                if (nlen >= sizeof(pkg_name)) nlen = sizeof(pkg_name) - 1;
                memcpy(pkg_name, entry->d_name, nlen);
                pkg_name[nlen] = '\0';
                runepkg_secure_strcpy(pkg_ver, sizeof(pkg_ver), ver_dash + 1);
            } else {
            ...
            if (pattern) {
                if (strncmp(pkg_name, pattern, pattern_len) != 0 &&
                    strncmp(entry->d_name, pattern, pattern_len) != 0) {
                    continue;
                }
            }
            ...
            for (j = 0; j < count; j++) {
                if (raw_names[j] && strcmp(raw_names[j], pkg_name) == 0) {
                    duplicate = true;
                    break;
                }
            }
            ...
            if (pkg_ver[0] != '\0') {
                snprintf(display_buf, sizeof(display_buf), "%s (%s)", pkg_name, pkg_ver);
            } else {
                runepkg_secure_strcpy(display_buf, sizeof(display_buf), pkg_name);
            }
            ...
            if (count >= capacity) {
                ...
                new_raw = realloc(raw_names, capacity * sizeof(char *));
                if (!new_packages || !new_raw) {
            ...
            packages[count] = strdup(display_buf);
            raw_names[count] = strdup(pkg_name);
            if (packages[count] && raw_names[count]) {
    ...
    for (i = 0; i < count; i++) {
        if (packages[i]) free(packages[i]);
        if (raw_names[i]) free(raw_names[i]);
    }
    free(packages);
    free(raw_names);
...
static int does_package_provide_virtual(const char *pkg_name, const char *virt_name) {
    PkgInfo info;
    if (!pkg_name || !virt_name) return 0;
    if (strcmp(pkg_name, virt_name) == 0) return 1;
            ...
            if (pcopy) {
                ...
                while (ptoken) {
                    ...
                    if (vname && vname[0] != '\0') {
                        ...
                        if (paren) *paren = '\0';
                        vname = runepkg_util_trim_whitespace(vname);
                        if (vname && strcmp(vname, virt_name) == 0) {
                            free(pcopy);
                            runepkg_pack_free_package_info(&info);
                            return 1;
                        }
...
static int is_virtual_dummy_provided_by(const char *virt_name, const char *pkg_name) {
    PkgInfo info;
    if (!virt_name || !pkg_name) return 0;
    if (runepkg_storage_read_package_info(virt_name, NULL, &info) == 0) {
        if ((info.section && strcmp(info.section, "virtual") == 0) ||
            (info.description && strstr(info.description, "Virtual dummy"))) {
            if (info.description && strstr(info.description, pkg_name)) {
                runepkg_pack_free_package_info(&info);
                return 1;
            }
        }
...
static int get_batch_or_installed_version(const char *target_pkg, char *out_ver, size_t ver_size) {
    if (!target_pkg || !out_ver) return 0;

    /* 1. Check if target_pkg is in g_batch_planned_packages */
    if (g_batch_planned_packages) {
        PkgInfo *planned = runepkg_hash_search(g_batch_planned_packages, target_pkg);
        if (planned && planned->version && planned->version[0] != '\0') {
            runepkg_secure_strcpy(out_ver, ver_size, planned->version);
            return 1;
        }
    }

    /* 2. Fall back to installed version on disk */
    return get_installed_package_version(target_pkg, out_ver, ver_size);
}
        ...
        if (entries[i].relation_type == RUNEPKG_RELATION_CONFLICTS) {
            if (strcmp(spkg, pkg_name) == 0) {
                if (strcmp(tpkg, pkg_name) != 0 &&
                    !does_package_provide_virtual(pkg_name, tpkg) &&
                    !is_virtual_dummy_provided_by(tpkg, pkg_name)) {
                    bool tpkg_in_batch = (g_batch_planned_packages && runepkg_hash_search(g_batch_planned_packages, tpkg) != NULL);
                    char inst_ver[128];
                    if (get_batch_or_installed_version(tpkg, inst_ver, sizeof(inst_ver))) {
                        bool is_conflict = true;
                        if (cons && cons[0] != '\0') {
                            is_conflict = (runepkg_util_check_version_constraint(inst_ver, cons) == 1);
                        } else if (tpkg_in_batch) {
                            is_conflict = false;
                        }
                        if (is_conflict) {
            ...
            if (strcmp(tpkg, pkg_name) == 0) {
                if (strcmp(spkg, pkg_name) != 0 &&
                    !does_package_provide_virtual(pkg_name, spkg) &&
                    !is_virtual_dummy_provided_by(spkg, pkg_name)) {
                    bool spkg_in_batch = (g_batch_planned_packages && runepkg_hash_search(g_batch_planned_packages, spkg) != NULL);
                    char inst_ver[128];
                    if (get_batch_or_installed_version(spkg, inst_ver, sizeof(inst_ver))) {
                        bool is_conflict = true;
                        if (cons && cons[0] != '\0' && pkg_version && pkg_version[0] != '\0') {
                            is_conflict = (runepkg_util_check_version_constraint(pkg_version, cons) == 1);
                        } else if (spkg_in_batch) {
                            is_conflict = false;
                        }
                        if (is_conflict) {
                            if (conflict_target && target_size > 0) {
            ...
            if (strcmp(tpkg, pkg_name) == 0 && strcmp(spkg, pkg_name) != 0 &&
                ...
                if (get_batch_or_installed_version(spkg, inst_ver, sizeof(inst_ver))) {
                    ...
                    if (spkg_in_batch) {
                        is_break = false;
                    } else if (cons && cons[0] != '\0' && pkg_version && pkg_version[0] != '\0') {
