...
static void supersede_host_dummy_packages(const PkgInfo *pkg_info) {
    ...
    snprintf(pattern, sizeof(pattern), "%s/host/%s", g_runepkg_db_dir, pkg_info->package_name);
    if (runepkg_util_file_exists(pattern)) {
        runepkg_util_log_verbose("[install] Native install superseding unversioned host dummy: %s", pattern);
        runepkg_storage_remove_directory_tree(pattern);
    }
    ...
    if (S_ISDIR(st.st_mode)) {
        ...
        if (getuid() == 0) {
            if (chown(dst, st.st_uid, st.st_gid) != 0) {
                /* ignore non-fatal directory chown error */
            }
        }
        ...
        if (len >= 0) {
            ...
            }
            {
                ...
                if (lstat(dst, &dst_st) == 0) {
                    if (S_ISDIR(dst_st.st_mode)) {
                        runepkg_storage_remove_directory_tree(dst);
                    } else {
                        unlink(dst);
                    }
                }
