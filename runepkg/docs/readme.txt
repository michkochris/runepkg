...
# This installs:
# - Binary to PREFIX/bin (default /usr/bin/runepkg)
# - System config to /etc/runepkg/runepkgconfig (when using sudo make install)
# Termux: paths use TERMUX_PREFIX as documented in the Makefile.
```
...
# Termux removal (same uninstall target; removes Termux paths when present)
make clean
make uninstall
...
*Built with ❤️ for the old-school GNU/Linux community — passion project around custom Linux-from-scratch workflows.*
