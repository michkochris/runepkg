    ...
    index = hash_function(pkg_info->package_name, table->size);
    {
        ...
        while (curr) {
            if (curr->data.package_name && strcmp(curr->data.package_name, pkg_info->package_name) == 0) {
                if (curr->data.version && (!pkg_info->version || runepkg_util_compare_versions(pkg_info->version, curr->data.version) < 0)) {
                    runepkg_util_log_verbose("Package '%s' in hash table already has version '%s' >= '%s', keeping existing.\n",
                                             pkg_info->package_name, curr->data.version, pkg_info->version ? pkg_info->version : "(none)");
                    return 0;
                }
