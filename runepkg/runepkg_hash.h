...
typedef struct PkgInfo {
    ...
    bool auto_installed;  /* NEW: Track if package was pulled in as a dependency */
} PkgInfo;
...
/**
 * @brief Checks if a package is provided as a virtual package in the hash table.
 * @param table A pointer to the hash table.
 * @param pkg_name The name of the package to check.
 * @return 1 if provided, 0 otherwise.
 */
