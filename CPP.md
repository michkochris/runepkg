...
## 3. Header Isolation & C FFI Boundary Guarantees

### Dual-Header Architecture
- **C++ Internal Headers (`.hpp`)**:
  Files like `runepkg_security.hpp` and `runepkg_util_cpp.hpp` contain modern C++17 interfaces (`std::string`, `std::filesystem`, `std::function`, classes, templates). These headers are included **exclusively** by C++ translation units (`.cpp`).
- **Master C FFI Bridge (`runepkg_cpp_ffi.h`)**:
  A pure ISO C89/C90 header (`extern "C"`) containing POD structs and C function prototypes. Included by C files (`runepkg_cli.c`, `runepkg_handle.c`, `runepkg_util.c`) when `ENABLE_CPP_FFI` is defined.

### 100% C FFI Exception Safety
C++ exceptions crossing into ISO C89 caller frames result in undefined behavior or process aborts. To guarantee complete stability, **every `extern "C"` function across all C++ files is wrapped in a `try { ... } catch (...)` guard block**:

```cpp
extern "C" int runepkg_security_verify_sha256(const char* filepath, const char* expected_hash) {
    try {
...
In `Makefile`, C++ sources and headers are configured as follows:

```makefile
ifeq ($(WITH_CPP),1)
  override CFLAGS  += -DENABLE_CPP_FFI
  LDFLAGS += $(shell pkg-config --libs libcurl openssl libcrypto 2>/dev/null || echo "-lcurl -lcrypto") -lz
  CPP_SOURCES = runepkg_security.cpp runepkg_util_cpp.cpp runepkg_resolver.cpp runepkg_network.cpp
  CPP_OBJECTS = $(CPP_SOURCES:.cpp=.o)
  LINKER = $(CXX)
endif
```

Executing `make all` compiles the C++ runes in exact foundational sequence:
1. `[ritual] Compiling C++ Rune: runepkg_security.cpp`
2. `[ritual] Compiling C++ Rune: runepkg_util_cpp.cpp`
3. `[ritual] Compiling C++ Rune: runepkg_resolver.cpp`
4. `[ritual] Compiling C++ Rune: runepkg_network.cpp`
